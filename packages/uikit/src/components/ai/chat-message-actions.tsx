...
const icon = (Fallback: (props: SVGProps<SVGSVGElement>) => ReactElement, slot: string) =>
  function Icon({ children, className, ...props }: IconProps): ReactElement {
    if (children && isValidElement(children))
      return cloneElement(children as ReactElement<{ className?: string; "data-slot"?: string }>, {
        ...props,
        className: ["size-4", className, (children.props as { className?: string }).className]
          .filter(Boolean)
          .join(" "),
        "data-slot": slot,
      });
      ...
      <LazyMotion features={domAnimation}>
        <AnimatePresence initial={false} mode="popLayout">
          <m.span
            ...
            initial={
              reduceMotion ? { opacity: 0 } : { filter: "blur(4px)", opacity: 0, scale: 0.25 }
            }
...
export function ChatMessageActionsAction({ className, ...props }: ActionProps): ReactElement {
  return <ChatMessage.Action {...props} {...(className === undefined ? {} : { className })} />;
}
const action = (Icon: (props: IconProps) => ReactElement) =>
  ...
  function Action({ children, className, ...props }: ActionProps): ReactElement {
    return (
      <ChatMessage.Action {...props} {...(className === undefined ? {} : { className })}>
