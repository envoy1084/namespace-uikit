---
title: Frameworks
description: Integrate Namespace UIKit with your framework
icon: layers
---
...
```bash
npx namespace-uikit-cli@latest init
```

<Callout>
  When prompted, select the **App** or **Pages** template. Then open your new
  folder and install dependencies (for example `pnpm install`).
</Callout>

### 2. Use your first Namespace UIKit component
    ...
    export default function HomePage() {
      return (
        <main className="flex min-h-screen items-center justify-center">
          <Button variant="tertiary">Hello Namespace UIKit</Button>
        </main>
      );
    }
    ```
...
<Callout>
  Namespace UIKit does not require a provider. Components work directly after
  installation and style import.
</Callout>
...
import { isRTL } from "@thenamespace/uikit";
...
import { I18nProvider } from "@thenamespace/uikit";
...
<Callout>
  When prompted, select the **Vite** template. Then open your new folder and
...
- [Quick Start](/docs/getting-started/quick-start) for the fastest setup path
- [Themes](/docs/getting-started/theming) to customize colors and tokens
- [Components](/docs/components) to explore all available components
