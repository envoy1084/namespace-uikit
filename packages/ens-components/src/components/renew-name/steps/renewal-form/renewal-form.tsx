...
import {
  ...
  NameRenewalHeader,
  NameRenewalHeading,
} from "#/components/renew-name/layout";
import { RenewalDetails } from "#/components/renew-name/steps/renewal-form/renewal-details";
import { useRenewalSubmission } from "#/components/renew-name/steps/renewal-form/use-renewal-submission";
...
export function NameRenewalForm({
  onPendingChange,
  onSuccess,
}: NameRenewalFormProps) {
  const {
    duration,
    input,
    isReferrerValid,
  ...
  const normalizedName = parsedInput.isOk()
  ...
  const hasMatchingQuote =
    quote.data !== undefined && quotedName === normalizedName;
  const canRenew =
    hasMatchingQuote &&
    !quote.isFetching &&
  ...
  return (
    <>
      <NameRenewalHeader className="mx-auto">
        {slots.formGraphic === undefined ? (
          <img
            ...
            src={DefaultNameRenewalGraphic.href}
          />
        ) : (
          slots.formGraphic
        )}
      ...
      <NameRenewalBody className="mt-2 flex-none">
        ...
        {!hasMatchingQuote &&
        (inputError !== undefined || quote.isFetching || quote.isError) ? (
          <div
            aria-live="polite"
      ...
      <NameRenewalFooter className="mt-5 flex-col">
        ...
        {submission.payment.data !== undefined &&
        !submission.payment.data.hasSufficientBalance ? (
          <Typography.Paragraph
