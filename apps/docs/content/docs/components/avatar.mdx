...
links:
  ...
  styles: avatar.css
  storybook: Components/Data Display/Avatar
  figma: true
---
...
Use `asChild` on `Avatar.Image` to compose with a custom image component. This example uses Next.js
`Image` for optimized loading. Pass `src` on `Avatar.Image` so it can track the loading state and
show the fallback until the image is ready.
