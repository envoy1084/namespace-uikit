---
title: Theming
description: Customize Namespace UIKit's design system with CSS variables and global styles
icon: palette
links:
  themes: true
  tailwind: theme
...
Namespace UIKit uses CSS variables and [BEM](https://getbem.com/) classes for theming. Customize everything from colors to component styles using standard CSS.

<Callout type="info">
  **Want to create your own theme?** Try the [Theme
  Builder](/docs/getting-started/theming) to visually customize colors, radius,
  fonts, and more — then export the CSS to use in your project.
</Callout>

## How It Works

Namespace UIKit's theming system is built on top of [Tailwind CSS v4](https://tailwindcss.com/docs/theme)'s theme. When you import `@thenamespace/uikit/styles`, it uses Tailwind's built-in color palettes, maps them to semantic variables, automatically switches between light and dark themes, and uses CSS layers and the `@theme` directive for organization.
...
> **Note**: See [Colors](/docs/getting-started/colors) for the complete color palette and visual reference.

> **Dark mode**: For a complete setup guide with `next-themes` and Namespace UIKit's `useTheme` hook, see [Dark Mode](/docs/getting-started/dark-mode).

**Create your own theme:**

<CollapsibleCode lang="css" code={`/* src/docs/getting-started/theming/ocean.css */
@layer base {
    /* Ocean Light */
    [data-theme="ocean"] {
      color-scheme: light;
...
@import "./src/docs/getting-started/theming/ocean.css" layer(theme); /* [!code highlight]*/
```
...
> **Note**: See [Styling](/docs/getting-started/styling) for the complete styling reference.
...
/* Base requirements */
@import "tailwindcss";
@import "@thenamespace/uikit/styles/base" layer(base);
/* OR specific base file */
@import "@thenamespace/uikit/styles/base/base.css" layer(base);

/* Theme variables */
@import "@thenamespace/uikit/styles/docs/getting-started/theming/shared/theme.css"
  layer(theme);
@import "@thenamespace/uikit/styles/docs/getting-started/theming/default"
  layer(theme);
/* OR specific theme files */
@import "@thenamespace/uikit/styles/docs/getting-started/theming/default/index.css"
  layer(theme);
@import "@thenamespace/uikit/styles/docs/getting-started/theming/default/variables.css"
  layer(theme);

/* Components (all components) */
@import "@thenamespace/uikit/styles/components" layer(components);
/* OR specific component files */
@import "@thenamespace/uikit/styles/components/index.css" layer(components);
@import "@thenamespace/uikit/styles/components/button.css" layer(components);
@import "@thenamespace/uikit/styles/components/accordion.css" layer(components);

/* Utilities (optional) */
@import "@thenamespace/uikit/styles/utilities" layer(utilities);

/* Variants (optional) */
@import "@thenamespace/uikit/styles/variants" layer(utilities);
```

> **Note**: Directory imports (e.g., `@thenamespace/uikit/styles/components`) automatically resolve to their `index.css` file. Use explicit file paths (e.g., `@thenamespace/uikit/styles/components/button.css`) to import individual component styles.

**Headless mode:** Build your own styles from scratch:

```css
@import "tailwindcss";
@import "@thenamespace/uikit/styles/base/base.css";
...
Namespace UIKit defines three types of variables in [`variables.css`](https://github.com/thenamespace/uikit/blob/v3/packages/styles/docs/getting-started/theming/default/variables.css):
...
For a complete reference, see: [Colors Documentation](/docs/getting-started/colors), [Default Theme Variables](https://github.com/thenamespace/uikit/blob/v3/packages/styles/docs/getting-started/theming/default/variables.css), [Shared Theme Utilities](https://github.com/thenamespace/uikit/blob/v3/packages/styles/docs/getting-started/theming/shared/theme.css)

**Tailwind theme bridge (`@theme inline`):**

[`themes/shared/theme.css`](https://github.com/thenamespace/uikit/blob/v3/packages/styles/docs/getting-started/theming/shared/theme.css) maps semantic variables to Tailwind tokens (`--color-*`, `--radius-*`, `--ease-*`). Calculated colors reference underlying vars (e.g. `--surface-hover`, `--accent-soft`) from `variables.css` — they are not inlined with `color-mix()` in this file:
...
Namespace UIKit applies a shared scrollbar style to component scroll areas (tables, popovers, drawers, and similar). Scrollbars use standard CSS properties (`scrollbar-width`, `scrollbar-color`, `scrollbar-gutter`) — no `::-webkit-scrollbar` overrides.

**Modes** — set `data-scrollbar` on `<html>`, a component root, or a scroll slot:

| Mode                 | `data-scrollbar`    | Behavior                                       |
| -------------------- | ------------------- | ---------------------------------------------- |
| Namespace UIKit thin | _(unset)_ or `thin` | Thin thumb from theme tokens                   |
...
<html data-scrollbar="default">
  <!-- Restore Namespace UIKit scrollbars in one subtree -->
  <main data-scrollbar="thin">...</main>

  <!-- Hide scrollbars in one subtree (e.g. a popover) -->
  <div data-scrollbar="none">...</div>
</html>
```

**Theme variables** — defined in light and dark theme blocks in [`variables.css`](https://github.com/thenamespace/uikit/blob/v3/packages/styles/docs/getting-started/theming/default/variables.css):
...
**Custom overflow areas** — use the `scrollbar`, `scrollbar-thin`, `scrollbar-default`, or `scrollbar-none` utilities from `@thenamespace/uikit/styles` on your own elements. See [Styling](/docs/getting-started/styling) for class-based overrides.

> **Note**: Some components hide scrollbars by default (date picker popovers, color picker, secondary tabs). Nested scroll slots (e.g. the calendar year picker inside a date picker) keep Namespace UIKit scrollbars because `scrollbar-none` only affects the element it is applied to, not descendants using `@apply scrollbar`.

## Vibrant Palette

By default, Namespace UIKit uses accessible soft foreground colors that mix the semantic color with the foreground for better contrast. If you prefer more saturated, vibrant soft foreground colors, add the `data-vibrant-palette` attribute to your root element:
...
<Callout type="warn">
  The vibrant palette prioritizes visual saturation over contrast. It may not
  meet WCAG accessibility guidelines for some color combinations, especially
  with lighter accent colors.
</Callout>

You can also enable vibrant palette in the [Theme Builder](/docs/getting-started/theming) via the "Vibrant palette" toggle in the theme popover.

## Resources

- [Colors Documentation](/docs/getting-started/colors)
- [Styling Guide](/docs/getting-started/styling)
