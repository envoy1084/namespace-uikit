...
export interface ReconcileRegistrationAttemptProps {
...
async function reconcileResolver(
  ...
  if (
    ...
    attempt.submission.type !== "resolver-pending"
  ) {
...
async function reconcileAtomicBatch(
  ...
  if (
    props.attempt.submission.type !== "atomic-pending" ||
    props.walletClient === undefined
  ) {
  ...
  if (
    commitment.value.state === "NOT_FOUND" ||
    commitment.value.state === "EXPIRED"
  ) {
...
export async function reconcileRegistrationAttempt(
  ...
  if (
    commitment.value.state === "WAITING" ||
    commitment.value.state === "READY"
  ) {
    const confirmedAt = Number(commitment.value.submittedAt) * 1_000;
    props.onUpdate({
  ...
  if (
    commitment.value.state === "EXPIRED" ||
    props.attempt.submission.type === "confirmed"
  ) {
  ...
  if (props.attempt.submission.type === "commitment-pending") {
    ...
    if (receipt.isErr()) {
      if (receipt.error === "TRANSACTION_REVERTED") {
        const resolverTransactionHash =
          props.attempt.submission.resolverTransactionHash;
        props.onUpdate({
          submission:
