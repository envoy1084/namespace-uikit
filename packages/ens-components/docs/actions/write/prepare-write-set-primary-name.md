...
Prepares an ENS `L2ReverseRegistrar.setName(string)` write. It does not submit
a transaction.
...
| `reverseRegistrarAddress` | `Address`                | Deployed `L2ReverseRegistrar` address.         |

`setName(string)` derives the address being named from `msg.sender`, so
`account` is both the submitting wallet and the address whose primary
