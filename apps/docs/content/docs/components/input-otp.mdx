...
```tsx
import { InputOTP } from "@thenamespace/uikit";
```
...
Use the `pattern` prop to restrict input to specific characters. Namespace UIKit exports common patterns like `REGEXP_ONLY_CHARS` and `REGEXP_ONLY_DIGITS`.
...
The InputOTP component uses these CSS classes ([View source styles](https://github.com/thenamespace/uikit/blob/main/packages/uikit/src/styles/components/input-otp.css)):
...
Namespace UIKit re-exports common regex patterns from input-otp for convenience:
