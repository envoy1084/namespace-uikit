...
import { prepareCommitNameWrite, preparePermissionedResolverDeploymentWrite } from "#/actions";
import type {
...
function phaseFor(prepared: PreparedContractWrite): CommitmentTransactionPhase {
  return prepared.kind === "deploy-permissioned-resolver" ? "resolver" : "commitment";
}
  ...
  return async (progress) => {
    ...
    if (progress.state === "submitted") {
      if (phase === "resolver") {
        ...
        props.onSubmissionChange({
          ...(resolverTransactionHash === undefined ? {} : { resolverTransactionHash }),
          transactionHash: progress.transactionHash,
...
function getResolverHash(attempt: StoredRegistrationAttempt): Hex | undefined {
  ...
  if (submission.type === "resolver-pending" || submission.type === "resolver-confirmed") {
    return submission.transactionHash;
  }
  if (submission.type === "commitment-pending" || submission.type === "confirmed") {
    return submission.resolverTransactionHash;
  }
  return undefined;
}

function buildSuccess(result: ExecuteContractWritesResult): CommitmentSubmissionSuccess {
  const confirmedAt = Date.now();
  const resolver = result.transactions.find(
    ({ prepared }) => prepared.kind === "deploy-permissioned-resolver",
  );
  const commitment = result.transactions.find(({ prepared }) => prepared.kind === "commit-name");

  return {
    ...(result.strategy === "atomic" ? { callsId: result.callsId } : {}),
    ...(commitment?.receipt === undefined ? {} : { commitmentReceipt: commitment.receipt }),
    confirmedAt,
    ...(resolver?.receipt === undefined ? {} : { resolverReceipt: resolver.receipt }),
    ...(resolver === undefined ? {} : { resolverTransactionHash: resolver.transactionHash }),
    ...(commitment === undefined ? {} : { transactionHash: commitment.transactionHash }),
  };
}
...
async function prepareResolver(
  ...
  const prepared = await preparePermissionedResolverDeploymentWrite(props.publicClient, {
  ...
  if (!isAddressEqual(prepared.value.metadata.resolverAddress, resolver.address)) {
...
export async function submitRegistrationAttempt(
  ...
  let calls: readonly [PreparedContractWrite, ...PreparedContractWrite[]] = [commitment.value];
