...
Namespace UIKit uses composition patterns to create flexible, customizable components. Change the rendered element, compose components together, and maintain full control over markup.

## Framework-Agnostic Styles

Namespace UIKit's variant functions are available in the `@thenamespace/uikit/styles` package, which can be used independently of React. This enables Vue, Svelte, and other frameworks to use Namespace UIKit's design system:

```tsx
// Import directly from @thenamespace/uikit/styles (framework-agnostic)
import { buttonVariants } from "@thenamespace/uikit/styles";

// Or import from @thenamespace/uikit (re-exports the same functions)
import { buttonVariants } from "@thenamespace/uikit";
```

Both imports work identically. Use `@thenamespace/uikit/styles` when building for non-React frameworks or when you want to avoid pulling in React dependencies.

## Polymorphic Styling

Apply Namespace UIKit styles to any element using variant functions or BEM classes. Extend component styles to framework components, native HTML elements, or custom components with full type safety.
...
When using a custom root element instead of Namespace UIKit's Root component, child components cannot access context slots. You can manually pass `className` to child components using variant functions or BEM classes:

```tsx
import { Link } from '@thenamespace/uikit';
import { linkVariants } from '@thenamespace/uikit/styles';
...
This approach works because Namespace UIKit's variant functions and BEM classes can be applied to any element, giving you complete flexibility to style framework components, native elements, or custom components with Namespace UIKit's design system.

## Direct Class Application

The simplest way to style links or other elements is to apply Namespace UIKit's [BEM](https://getbem.com/) classes directly. This approach is straightforward and works with any framework or vanilla HTML.
...
This approach works because Namespace UIKit uses [BEM](https://getbem.com/) classes that can be applied to any element. It's perfect when you don't need the component's interactive features (like `onPress` handlers) and just want the visual styling.

## Using Variant Functions

For more control and type safety, use variant functions to apply Namespace UIKit styles to framework-specific components or custom elements. Variant functions are available from both `@thenamespace/uikit/styles` (framework-agnostic) and `@thenamespace/uikit` (re-exports).
...
**Available variant functions:** Each component exports its variant function (`buttonVariants`, `chipVariants`, `linkVariants`, `spinnerVariants`, and more) from `@thenamespace/uikit/styles`. Use them to apply Namespace UIKit's design system to any element while maintaining type safety.

## Compound Components

Namespace UIKit components are built as compound components—they export multiple parts that work together. Use them in three flexible ways:
...
```tsx
import { Alert, AlertTitle, AlertDescription } from "@thenamespace/uikit";
...
// Or named export
import { ButtonRoot } from '@thenamespace/uikit';
...
```tsx
import { Link } from "@thenamespace/uikit";
import { linkVariants, buttonVariants } from "@thenamespace/uikit/styles";

// Link styled with button variants
const buttonStyles = buttonVariants({ variant: "tertiary", size: "md" });

<Link className={buttonStyles} href="https://namespace-uikit.com">
  Namespace UIKit
</Link>;
```
...
Create your own components by composing Namespace UIKit primitives:
...
```tsx
import type { ButtonRootProps } from "@thenamespace/uikit";
import type { VariantProps } from "tailwind-variants";

import { Button } from "@thenamespace/uikit";
import { buttonVariants, tv } from "@thenamespace/uikit/styles";
...
```tsx
import type { ButtonRootProps, AvatarRootProps } from "@thenamespace/uikit";
...
```tsx
import { Button, Avatar } from "@thenamespace/uikit";
...
Since `@thenamespace/uikit/styles` has no React dependencies, you can use it directly in any framework:
...
- Learn about [Styling](/docs/getting-started/styling) components
- Explore [Animation](/docs/getting-started/animation) options
