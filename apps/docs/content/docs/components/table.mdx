...
Namespace UIKit's Table works as a rendering layer on top of headless table libraries.
This example uses [TanStack Table](https://tanstack.com/table) for column
definitions, sorting, and pagination — while Namespace UIKit handles styling and
accessibility.
...
The Table component uses these CSS classes ([View source styles](https://github.com/thenamespace/uikit/blob/main/packages/uikit/src/styles/components/table.css)):
