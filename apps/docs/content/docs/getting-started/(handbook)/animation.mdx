---
title: Animation
description: Add smooth animations and transitions to Namespace UIKit components
icon: copy-transparent
---

Namespace UIKit components support multiple animation approaches: built-in CSS transitions, custom CSS animations, and JavaScript libraries like Framer Motion.

## Built-in Animations

Namespace UIKit components use data attributes to expose their state for animation:
...
Namespace UIKit components work seamlessly with Framer Motion for advanced animations.
...
**Respecting motion preferences:** Namespace UIKit automatically respects user motion preferences using Tailwind's `motion-reduce:` utility. All built-in transitions and animations are disabled when users enable "reduce motion" in their system settings.

Namespace UIKit extends Tailwind's `motion-reduce:` variant to support both the native `prefers-reduced-motion` media query and the `data-reduce-motion` attribute.

```css
/* Namespace UIKit pattern - uses Tailwind's motion-reduce: */
.button {
...
<html data-reduce-motion="true">
  <!-- All Namespace UIKit animations will be disabled -->
</html>
```

Namespace UIKit automatically detects the user's `prefers-reduced-motion: reduce` setting and disables animations accordingly.
...
- Learn about [Styling](/docs/getting-started/styling) approaches
- Explore [Component](/docs/components) examples
- View [Theming](/docs/getting-started/theming) documentation
