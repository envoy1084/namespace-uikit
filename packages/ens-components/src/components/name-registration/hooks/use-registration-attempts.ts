...
export type RegistrationAttemptUpdate = Partial<Omit<StoredRegistrationAttempt, "id">>;

export type StoredRegistrationAttempts = Record<string, StoredRegistrationAttempt>;
...
function resolverMatches(
  ...
  return resolver.type === "custom" && isAddressEqual(resolver.address, resolverAddress);
}
...
export function useRegistrationAttempts() {
  ...
  const find = useCallback(
    (input: FindRegistrationAttemptInput) => {
      ...
      const attempt = Object.values(attempts).reduce<StoredRegistrationAttempt | undefined>(
        (latest, candidate) => {
          const matches =
          ...
          if (!matches || (latest !== undefined && latest.updatedAt > candidate.updatedAt)) {
            return latest;
          }
