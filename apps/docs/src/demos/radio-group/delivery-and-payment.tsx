...
export function DeliveryAndPayment() {
  ...
  return (
    <div
      ...
      style={{
        // @ts-expect-error -- Demo intentionally overrides component CSS custom properties.
        "--accent": "#006FEE",
          ...
          <div className="grid gap-x-4 md:grid-cols-3">
            {deliveryOptions.map((option) => (
              <Radio key={option.value} value={option.value}>
                <Radio.Content
                  className={clsx(
                    "group bg-surface data-[selected=true]:border-accent data-[selected=true]:bg-accent/10 relative flex w-full flex-col gap-6 rounded-xl border border-transparent px-5 py-4 transition-all",
          ...
          <div className="grid gap-x-4 md:grid-cols-2">
            {paymentOptions.map((option) => (
              <Radio key={option.value} value={option.value}>
                <Radio.Content
                  className={clsx(
                    "group bg-surface relative flex w-full flex-row items-start justify-start gap-4 rounded-xl border border-transparent px-5 py-4 transition-all",
