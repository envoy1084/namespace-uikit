...
export interface PrepareRegistrationAttemptParameters {
  account: Address;
  chainId: number;
  duration: bigint;
...
function buildRegistrationAttempt(
  props: PrepareRegistrationAttemptParameters,
  resolver: StoredRegistrationResolver,
  secret: Hex,
): Result<RegistrationAttemptInput, MakeNameCommitmentError | ParseNameInputError> {
...
export function prepareRegistrationAttempt(
  ...
  if (props.resolverAddress !== null) {
    ...
    return isResolverDeployed(publicClient, resolverAddress).andThen((isDeployed) => {
