export { diffProfileRecords } from "#/components/name-profile-editor/diff-profile-records";
export { NameProfileEditor } from "#/components/name-profile-editor/name-profile-editor";
export { normalizeProfileRecords } from "#/components/name-profile-editor/normalize-profile-records";
export { DEFAULT_NAME_PROFILE_EDITOR_MESSAGES } from "#/components/name-profile-editor/customization";
export type { NameProfileEditorProps } from "#/components/name-profile-editor/name-profile-editor";
export type {
  NameProfileEditorMessages,
  NameProfileEditorPresentation,
  NameProfileEditorSlots,
} from "#/components/name-profile-editor/customization";
export type {
  NameProfileEditorErrorEvent,
  NameProfileEditorErrorPhase,
  NameProfileEditorEventHandler,
  NameProfileEditorEvents,
  NameProfileUpdateEvent,
} from "#/components/name-profile-editor/events";
export type {
  NormalizeProfileRecordsError,
  NormalizeProfileRecordsResult,
} from "#/components/name-profile-editor/normalize-profile-records";
export type {
  NameProfileEditorReview,
  NameProfileAbiRecord,
  NameProfileAbiRecordChange,
  NameProfileAddressRecord,
  NameProfileAddressRecordChange,
  NameProfileContenthashRecordChange,
  NameProfileDataRecord,
  NameProfileDataRecordChange,
  NameProfileEditorState,
  NameProfileEditorView,
  NameProfileFormValues,
  NameProfileInterfaceRecord,
  NameProfileInterfaceRecordChange,
  NameProfileImageRecord,
  NameProfileImageUpload,
  NameProfileImageUploadContext,
  NameProfileNameRecordChange,
  NameProfilePublicKeyRecord,
  NameProfilePublicKeyRecordChange,
  NameProfileRecordChange,
  NameProfileRecordType,
  NameProfileTextRecord,
  NameProfileTextRecordChange,
} from "#/components/name-profile-editor/types";
export { emptyNameProfileFormValues } from "#/components/name-profile-editor/types";
