...
export function TimerStep({ isCompleted = false, isDisabled = true, onComplete }: TimerStepProps) {
  const { registrationAttemptId, setRegistrationAttemptId } = useNameRegistration();
  ...
  const progress =
    ((COMMITMENT_WAIT_DURATION_SECONDS - remainingSeconds) / COMMITMENT_WAIT_DURATION_SECONDS) *
    100;
          ...
          <Accordion.Body className="px-4 pt-2 pb-4 text-center">
            <div className="relative mx-auto size-28">
              <ProgressCircle
                ...
                value={progress}
              >
                <ProgressCircle.Track className="size-28!" strokeWidth={3} viewBox="0 0 36 36">
              ...
              <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                <NumberFlowGroup>
                  <span className="text-foreground font-mono text-xl font-medium tabular-nums">
                    <NumberFlow format={TIMER_NUMBER_FORMAT} trend={-1} value={minutes} />
                    <span aria-hidden="true">:</span>
                    <NumberFlow format={TIMER_NUMBER_FORMAT} trend={-1} value={seconds} />
                  </span>
                </NumberFlowGroup>
              </div>
            </div>
            <Typography.Paragraph
              ...
              size="xs"
            >
              This short waiting period protects your registration from front-running.
            </Typography.Paragraph>
          </Accordion.Body>
        </Accordion.Panel>
      ) : null}
